...
def test_keycloak_ui_reachability_customized_hostname_via_override(openshift_dyn_client):
    """Customize Keycloak hostname via values-hub overrides (rhbk-chart)."""
    customized_hostname = "keycloak-customized"
    logger.info("Checking reachability with customized hostname via values-hub override")
    app_route = get_route_by_app_label(
        openshift_dyn_client=openshift_dyn_client,
        project=app_project_name,
        label=app_label,
...
def modify_keycloak_hostname_via_hub_overrides(values_hub_path, hostname):
    ...
    """
    logger.info("Modify Keycloak hostname via clusterGroup.applications.rh-keycloak.overrides")
    ...
    if "rh-keycloak" not in apps:
        raise KeyError("clusterGroup.applications.rh-keycloak not found in values-hub.yaml")
